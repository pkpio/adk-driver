...
#define DEBUG_TAG "Adk-diver"
#define ADK_MINOR_BASE	96 /* Base address for the minor number */
	 ...
	 size_t count, loff_t *ppos)
{
	...
	printk("Adk-driver: Bulk read return code: %d\n", retval);
...
adk_probe (struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	/* Device attached in accessory mode */
		...
		printk("New ADK device attached to /dev/adk%d\n",
				interface->minor - ADK_MINOR_BASE);
